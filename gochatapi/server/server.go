...
// CORSMiddleware ...
func (s *Server) CORSMiddleware(handler routing.Handler) routing.Handler {
	return func(c *routing.Context) error {
...
func (s *Server) GetRouting() *routing.Router {
	...
	auth.Post("/reg", s.CORSMiddleware(s.RegistrationHandler()))
	auth.Post("/login", s.CORSMiddleware(s.LoginHandler()))

	user := router.Group("/users")
	user.Get("/me", s.CORSMiddleware(s.AuthenticationMiddleware(s.GetCurrentUser())))
